...
function show(button_element) {
    ...
    */
    ...
    for (var attribute in facts) {
        if (attribute == id) {
            var fun_fact = facts[attribute];
        }
    }

    document.getElementById('ta-fun-fact').innerText = fun_fact;
    ...
    document.getElementById('ta-photo').setAttribute('src', './photos/' + id + '.jpg');
    ...
    for (attribute in facts) {
        if (id != attribute) {
            document.getElementById(attribute).style.backgroundColor = 'darkblue';
